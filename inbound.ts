// Legacy inbound bridge: forward messages sent *to* the Telegram bot on to Discord/Telegram
// channels configured via env. Kept for backward compatibility; enabled with ENABLE_INBOUND_BOTS.
...
async function beforeTelegramSend(
    ...
    const hasDiscordChannels = discordChannelIds
        .split(',')
        .some((id) => id.trim());
    ...
    if (telegramChunks && telegramChunks.length > 0) {
        const extractedContent = { text, isFormatted: true };
...
console.log('Legacy inbound bots initialized.');
