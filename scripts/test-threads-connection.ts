...
const DEFAULT_LOGIN_URL = 'https://www.threads.net/login';
const DEFAULT_HOME_URL = 'https://www.threads.net/';
...
const timeoutMs = envNumber('THREADS_LOGIN_TIMEOUT_MS', DEFAULT_TIMEOUT_MS);
const reuseState = existsSync(storageStatePath) && !envFlag('THREADS_TEST_RESET');
...
console.log('Starting Threads connection smoke test');
...
try {
    ...
    while (Date.now() - startedAt < timeoutMs) {
        if (await threadsLoginDetector.isLoggedIn(page).catch(() => false)) {
        ...
        if (!reuseState) {
            console.log('Waiting for Threads login to finish...');
        }
        await sleep(POLL_MS);
    }
    ...
    if (connected) {
        ...
        console.error(
            reuseState
                ? 'Stored Threads session is not connected. ' +
                      'Run with THREADS_TEST_RESET=true to log in again.'
                : 'Timed out waiting for Threads login.',
        );
        process.exitCode = 1;
    }
} catch (error) {
